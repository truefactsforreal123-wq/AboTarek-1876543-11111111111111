    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-extrabold text-admin-text">إدارة المنيو</h1>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-admin-border bg-admin-surface p-3">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-admin-text-muted/50" />
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="ابحث عن عنصر..."
            className="w-full rounded-md border border-admin-border bg-admin-input-bg py-2.5 pr-10 pl-3 text-sm outline-none focus:border-brand-500"
          />
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`rounded-md px-3 py-2 text-xs font-bold transition-colors ${
              cat === "all" ? "bg-admin-text text-admin-surface" : "bg-admin-surface-hover text-admin-text-muted hover:bg-admin-border/50"
            }`}
          >
            الكل ({items.length})
          </button>
          {cats.map((c) => (
            <button
              ...
              onClick={() => setCat(c.id)}
              className={`rounded-md px-3 py-2 text-xs font-bold transition-colors ${
                cat === c.id ? "bg-admin-text text-admin-surface" : "bg-admin-surface-hover text-admin-text-muted hover:bg-admin-border/50"
              }`}
            >
              {c.name}
            </button>
      ...
      <div className="overflow-hidden rounded-lg border border-admin-border bg-admin-surface">
        <div className="hidden grid-cols-12 gap-4 border-b border-admin-border bg-admin-surface-hover/50 px-5 py-3 text-xs font-bold uppercase tracking-wider text-admin-text-muted/70 sm:grid">
              ...
              >
                ...
                <div className="col-span-2 flex items-center gap-3 sm:col-span-5">
                  {it.image ? (
                    ...
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-admin-surface-hover text-xs font-bold text-admin-text-muted/50">
                      —
                    </div>
                  )}
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="truncate text-sm font-extrabold text-admin-text">
                        {it.nameAr}
                      </span>
                      {it.badge === "signature" && (
                        <Star className="h-3.5 w-3.5 shrink-0 text-amber-500" fill="currentColor" />
                      )}
                      {it.badge === "spicy" && <Flame className="h-3.5 w-3.5 shrink-0 text-red-500" />}
                      {it.badge === "popular" && (
                        <span className="rounded bg-red-50 px-1.5 py-0.5 text-[0.6rem] font-bold text-red-600">
                          طلب كثير
                        </span>
                      )}
                    </div>
                    <p className="mt-0.5 truncate text-xs text-admin-text-muted/70">{it.descAr}</p>
                  </div>
                </div>
                {/* category */}
                <div className="col-span-1 text-xs text-admin-text-muted sm:col-span-2">
                  {it.categoryName}
                </div>
                {/* price */}
                <div className="col-span-1 text-sm font-extrabold tabular-nums text-brand-600 sm:col-span-2">
                ...
                <div className="col-span-1 sm:col-span-1">
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[0.65rem] font-bold transition-colors disabled:opacity-40 ${
                      avail ? "bg-blue-50 text-blue-700" : "bg-gray-100 text-gray-500"
                    }`}
                  >
                    {actingId === it.id ? (
                      <span className="h-3 w-3 animate-spin rounded-full border border-blue-700/30 border-t-blue-700" />
                    ) : avail ? (
                ...
                <div className="col-span-2 flex justify-start gap-1 sm:col-span-2 sm:justify-end">
                  ...
                  <button
                    ...
                    disabled={actingId === it.id}
                    className="rounded-md p-2 text-red-500 transition-colors hover:bg-red-50 disabled:opacity-40"
                    aria-label="حذف"
                  >
                    {actingId === it.id ? (
                      <span className="h-4 w-4 animate-spin rounded-full border-2 border-red-500/30 border-t-red-500 block" />
                    ) : (
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed inset-x-4 top-1/2 z-50 -translate-y-1/2 rounded-lg bg-admin-surface shadow-2xl sm:inset-x-auto sm:left-1/2 sm:w-full sm:max-w-lg sm:-translate-x-1/2"
        dir="rtl"
      >
        <div className="flex items-center justify-between border-b border-admin-border px-6 py-4">
          <h2 className="text-lg font-extrabold text-admin-text">
