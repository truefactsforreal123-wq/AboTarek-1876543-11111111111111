...
export function DownloadTableQRPDF({
  ...
  async function downloadPDF() {
    ...
    try {
      ...
      const qrDataUrl = await QRCode.toDataURL(url, {
        ...
        color: { dark: "#0e0707", light: "#ffffff" },
      });
  ...
  return (
    <button
      onClick={downloadPDF}
      disabled={loading}
      className="flex items-center gap-1.5 rounded-lg bg-admin-surface-hover px-3 py-2 text-xs font-bold text-admin-text-muted hover:bg-admin-border/50 hover:text-admin-text transition-colors disabled:opacity-40"
    >
      <FileDown size={14} />
      {loading ? "..." : "PDF"}
    </button>
