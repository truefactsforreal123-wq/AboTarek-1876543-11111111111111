...
-- Schedule the job to run every hour (skip if already exists)
SELECT cron.unschedule('cleanup-old-served-orders') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'cleanup-old-served-orders'
);
